...
def basicNaiveBayesModel( data, query, featureList, targetName, targetLevels, method, datatype = "discrete", boundaryList = None):
	...
	for target in targetLevels:
		...
		for idx in range(0,len(featureList)):
			...
			# NORMAL
			if method == "normal":
				u = targetSubset[feature].mean()
				o = targetSubset[feature].std()
				total *= normalDist( query[idx], u, o )
				print(f"Feature: {feature}, mean: {u}, std: {o}, normal distribution: {normalDist(query[idx], u, o)}")
			if method == "printnormal":
				u = targetSubset[feature].mean()
				o = targetSubset[feature].std()
				print(f"Feature: {feature}, mean: {u}, std: {o}")
			# EXPONENTIAL
			elif method == "exp":
